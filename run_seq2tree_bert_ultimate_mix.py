...
"""
print("the size of train_pairs:{}".format(len(train_pairs)))

train_pairs = train_pairs + valid_pairs
print("After mixup the size of train_pairs:{}".format(len(train_pairs)))
# Initialize models
...
for epoch in range(1, config.n_epochs+1):
    ...
    print("dropout:{} contra_weight:{} is_mask:{} is_em_dropout:{} is_prune2test:{} prunePercent:{} embedding_size:{} hidden_size:{}  warm_up_strategy:{} model_name:{} quantity_num:{}".format(config.dropout, config.contra_weight, config.is_mask, config.is_em_dropout, config.is_prune2test, config.prunePercent, config.embedding_size, config.hidden_size, config.warm_up_stratege, config.MODEL_NAME, config.quantity_num))
    print("------------------------------------------------------")
